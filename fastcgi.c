...
void qwq_fastcgi_clear(qwq_fastcgi *qwq)
{
    ...
    if (qwq->std_in)
        qwq_string_clear(qwq->std_in);
    if (qwq->std_out)
        qwq_string_clear(qwq->std_out);
    if (qwq->data)
        qwq_string_clear(qwq->data);
    if (qwq->params)
        qwq_hashmap_clear(qwq->params);
}

void qwq_fastcgi_destroy(qwq_fastcgi *qwq)
{
    if (qwq->std_in)
        qwq_string_destroy(qwq->std_in);
    if (qwq->std_out)
        qwq_string_destroy(qwq->std_out);
    if (qwq->data)
        qwq_string_destroy(qwq->data);
    if (qwq->params)
        qwq_hashmap_destroy(qwq->params);
    free(qwq);
}
...
static int parse_fcgi_params(int fd, qwq_fastcgi *qwq)
{
    ...
    {
        if (key)
            qwq_string_destroy(key);
        if (val)
            qwq_string_destroy(val);
        if (qmap)
            qwq_hashmap_destroy(qmap);
        qwq->params = NULL;
        return -1;
    }
               ...
               void **func_params)
{
    ...
    while (1)
    {
        ...
        case FCGI_END_REQUEST:
            ...
            if (response_handle)
                response_handle(connfd, fcgi, func_params);
            f_end = 1;
            break;
        }
    }
